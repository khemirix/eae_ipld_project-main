...
n_countries = len(pd.Series(movies_df['country'].str.split(', ').sum()).unique())

# TODO: Ex 2.5: How many characters long are on average the title names?
avg_title_length = movies_df['title'].apply(len)
...
year_data = movies_df[movies_df['release_year'] == year]

country_counts = year_data.groupby('country')['type'].value_counts().unstack().fillna(0).sum(axis=1)
