...
# ----- Page configs -----
st.set_page_config(
    page_title="Aymen Khemiri Portfolio",
    page_icon="📊",
)
...
min_temp_city = temps_df.City[temps_df['AvgTemperatureCelsius'].idxmin()]  
min_temp_date = temps_df.Date[temps_df['AvgTemperatureCelsius'].idxmin()]  

max_temp_city = temps_df.City[temps_df['AvgTemperatureCelsius'].idxmax()]  
max_temp_date = temps_df.Date[temps_df['AvgTemperatureCelsius'].idxmax()] 
...
if unique_countries_list is not None and len(selected_cities) > 0:
    ...
    for city in selected_cities:
        city_df = temps_df[temps_df['City'].isin(selected_cities)]            # TODO: get a dataframe with the rows of the selected city
        city_df_period =  city_df[(temps_df['Date'] >= start_date) & (temps_df['Date'] <= end_date) ]    # TODO: get a dataframe with the rows of the selected city and the selected period of time using the Date column and any of the <, >, <=, >= operators to compare with start_date and end_date
